...
COMMAND_TABLE_ARRAY_ADDR = 0x00B3D4A8
...
def analyze_function(func_addr):
    ...
    try:
        ...
        return {k: v for k, v in sorted(parameter_map.items())}, return_type
    except Exception as e:
...
def dump_command_table(category, index, table_addr, count):
    ...
    for i in range(count):
        ...
        parameters = [
            {"Type": param_type, "Name": f"param{index+1}", "Description": ""}
            for index, param_type in parameter_map.items()
        ]
