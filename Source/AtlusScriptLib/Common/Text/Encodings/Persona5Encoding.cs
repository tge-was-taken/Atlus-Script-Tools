...
namespace AtlusScriptLib.Common.Text.Encodings
{
    public class Persona5Encoding : Encoding
    {
        ...
        static Persona5Encoding()
        {
            // build character to codepoint table
            sCharToCodePoint = new Dictionary<char, CodePoint>( sCharTable.Length );

            // add the ascii range seperately
            for ( int charIndex = 0; charIndex < ASCII_RANGE + 1; charIndex++ )
            {
                if ( !sCharToCodePoint.ContainsKey( sCharTable[charIndex] ) )
                    sCharToCodePoint[sCharTable[charIndex]] = new CodePoint( 0, ( byte )charIndex );
            }

            // add extended characters, but don't re-include the ascii range
            for ( int charIndex = ASCII_RANGE + 1; charIndex < sCharTable.Length; charIndex++ )
            {
                ...
                if ( !sCharToCodePoint.ContainsKey( sCharTable[charIndex] ) )
                    sCharToCodePoint[sCharTable[charIndex]] = new CodePoint( ( byte )( GLYPH_TABLE_INDEX_MARKER | tableIndex ), ( byte )( tableRelativeIndex ) );
            }

            // build code point to character lookup table
            sCodePointToChar = new Dictionary<CodePoint, char>( sCharTable.Length );

            // add the ascii range seperately
            for ( int charIndex = 0; charIndex < ASCII_RANGE + 1; charIndex++ )
            {
                sCodePointToChar[new CodePoint( 0, ( byte )charIndex )] = sCharTable[charIndex];
            }

            // add extended characters, and make sure to include the ascii range again due to overlap
            for ( int charIndex = 0x20; charIndex < sCharTable.Length; charIndex++ )
            {
                int glyphIndex = charIndex + CHAR_TO_GLYPH_INDEX_OFFSET;
                int tableIndex = ( glyphIndex / GLYPH_TABLE_SIZE ) - 1;
                int tableRelativeIndex = glyphIndex - ( tableIndex * GLYPH_TABLE_SIZE );
