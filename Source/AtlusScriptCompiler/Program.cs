...
using AtlusScriptLib.Shared;
using AtlusScriptLib.FlowScript;
using AtlusScriptLib.FlowScript.Disassembler;
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (options.ContainsKey(OPTION_IDENTIFIER_DISASSEMBLE))
            {
                ...
                var result = FlowScriptBinary.LoadFromFile(inputPath, out FlowScriptBinary script);
                if (result != FlowScriptBinaryLoadResult.OK)
                {
                    Console.WriteLine($"FlowScript binary failed to load. Result code: {result}");
                    Console.ReadKey();
                    return;
                }
                else
